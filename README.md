...
But when a Transmitter 3-position toggle switch is flicked, the STM enters an Auto mode:
 ...
 - Altitude + Velocity hold mode -> Throttle, Roll & Pitch modified to maintain height and Velocity setpoint

Separate PID algorithms run to control for x, y & z. Height is measured using the TFmini LIDAR module, x,y velocity is measured using the Px4Flow sensor.
...
- Read current and voltage of the battery via ADC1
...
- We prevent PID windup when sat on the floor by zeroing the integral PID parts when throttle is < 1100us for > 1 second.
...
To Do:
 - Tune X & Y PIDs for velocity hold

 - Get UART from Pi working
 - Allow Pi to send X,Y velocity setpoint changes 
